...
void forward_cpu(float* out, float* preatt, float* att,
                       const float* inp,
...
__global__ void forward_fused1(float* out, float* preatt, float* att,
                                         const float* inp,
                                         int B, int T, int C, int NH) {
    // input is (B, T, 3C) Q,K,V
    // preatt, att are (B, NH, T, T)
...
void forward2(float* out,
                       const float* inp,
                       int B, int T, int C, int NH,
                       const int block_size) {
    // TODO there should be no mallocs inside any of these functions!
    // not fixing this because we don't intend to use forward2,
...
void forward3(float* out, float* vaccum, float* qkvr, float* preatt, float* att,
                       const float* inp,
                       int B, int T, int C, int NH,
                       const int block_size) {
    // inp is (B, T, 3C) QKV
...
void forward4(float* out, float* vaccum, float* qkvr, float* preatt, float* att,
                        const float* inp,
                        int B, int T, int C, int NH,
                        const int block_size) {
    // inp is (B, T, 3C) QKV
...
void forward5(float* out, floatX* vaccum, floatX* qkvr, floatX* preatt, floatX* att,
                        const float* inp,
                        int B, int T, int C, int NH,
...
void forward_cudnn(floatX* out,  // output: (B, T, NH, HS)
...
void forward(int kernel_num,
    ...
    switch (kernel_num) {
        case 1:
            forward1(out, preatt, att, inp, B, T, C, NH, block_size);
            break;
        case 2:
            forward2(out, inp, B, T, C, NH, block_size);
            break;
        case 3:
            forward3(out, vaccum, qkvr, preatt, att, inp, B, T, C, NH, block_size);
            break;
        case 4:
            forward4(out, vaccum, qkvr, preatt, att, inp, B, T, C, NH, block_size);
            break;
        case 5:
            forward5(out, (floatX*)vaccum, (floatX*)qkvr,
        ...
        case 10:
            ...
            forward_cudnn((floatX*)vaccum, stats, (floatX*)qkvr, inp, out, B, T, C, NH);
            break;
        #endif
        default:
